...
import { parseScanRows, scanColumns, selectScanSheet } from "./scan-data";
...
export default function Home() {
  ...
  const [scanSheets, setScanSheets] = useState<{ name: string; rows: BoxRow[] }[]>([]);
  ...
  async function readFile(file: File, kind: "system" | "scan") {
    try {
      ...
      if (kind === "scan") {
        const candidates = wb.SheetNames.map(name => ({ name, rows: XLSX.utils.sheet_to_json<unknown[]>(wb.Sheets[name], { header: 1, defval: "", raw: false }) }));
        const selected = selectScanSheet(candidates);
        if (!selected) throw new Error("No scan worksheet found · 未找到扫描主表，请使用库位箱号工作表");
        sheetName = selected;
        setScanSheets(candidates.filter(sheet => scanColumns(sheet.rows)).map(sheet => ({ name: sheet.name, rows: parseScanRows(sheet.rows) })));
      }
  ...
  return <main><aside className="sidebar"><div className="brand"><span>R</span><div><b>RUNERGY</b><small>Daily Inbound Verification · 每日入库核验</small><small className="app-version">版本 v{APP_VERSION}</small></div></div><nav><a className="active">⌁ <span>Box Reconciliation<small>逐箱核对</small></span></a><a>▦ <span>Count History<small>盘点历史</small></span></a><a>◎ <span>Location Analysis<small>库位分析</small></span></a><a>⚙ <span>Field Settings<small>字段设置</small></span></a></nav><div className="side-card"><div className="pulse"/><b>Processed on this device</b><small>数据仅在本机处理</small><p>Your inventory files never leave this browser.<br/><small>上传文件不会离开浏览器。</small></p></div><div className="operator"><span>JW</span><div><b>Warehouse Admin</b><small>仓库管理员 · Daily Count</small></div></div></aside>
    <section className="workspace"><header><div><p className="eyebrow">BOX INVENTORY RECONCILIATION <small>逐箱库存核对</small></p><h1>Every box, in the right place.</h1><p className="title-zh">每一个箱，都在正确的位置。</p><p className="subtitle">Connect system inventory to physical scans by box number.<small>通过箱号连接系统库存与现场扫描，定位错库位、漏扫与未入账。</small></p></div><div className="date"><small>SCAN WORKSHEET<br/><i>扫描工作表</i></small><b>{scanSheet}</b>{scanSheets.length > 1 && <select aria-label="Scan worksheet / 扫描工作表" value={scanSheet} onChange={(event) => { const selected = scanSheets.find(sheet => sheet.name === event.target.value); if (selected) { setScan(selected.rows); setScanSheet(selected.name); setNotice(`Sheet ${selected.name} · 已读取 ${selected.rows.length.toLocaleString()} 条扫描记录`); } }}>{scanSheets.map(sheet => <option key={sheet.name} value={sheet.name}>{sheet.name} ({sheet.rows.length.toLocaleString()})</option>)}</select>}</div></header>
      ...
      <section className="results"><div className="results-head"><div><p className="eyebrow">RECONCILIATION RESULTS <small>核对结果</small></p><h2>Box-Level Exceptions</h2><p className="section-zh">逐箱差异明细</p></div><button className="export" onClick={exportResults}>⇩ Export Action List <small>导出处理清单</small></button></div><div className="toolbar"><div className="filters">{[["all","All","全部"],["misplaced","Wrong Location","库位不符"],["unregistered","Not in System","系统无此箱"],["missing","Not Scanned","未扫描"],["duplicate","Duplicate","重复扫描"],["match","Matched","一致"]].map(([v,en,zh])=><button key={v} className={filter===v?"selected":""} onClick={()=>setFilter(v)}>{en}<small>{zh}</small></button>)}</div><label className="search">⌕<input value={query} onChange={(e)=>setQuery(e.target.value)} placeholder="Search box or location · 搜索箱号或库位"/></label></div><div className="table-wrap"><table><thead><tr><th>Box Number<small>箱号</small></th><th>System Location<small>系统库位</small></th><th>Scanned Location<small>扫描库位</small></th><th>Scans<small>扫描次数</small></th><th>Status<small>状态</small></th><th>Recommended Action<small>处理建议</small></th></tr></thead><tbody>{visible.map((r)=><tr key={`${r.box}-${r.status}`}><td><b>{r.box}</b></td><td><code>{r.systemLocation}</code></td><td><code>{r.scannedLocation}</code></td><td>{r.scans}</td><td><span className={`status ${r.status}`}>{statusText[r.status]}<small>{statusZh[r.status]}</small></span></td><td className="suggestion"><b>{r.action}</b><small>{r.actionZh}</small></td></tr>)}</tbody></table>{!visible.length&&<div className="empty">No records match this filter.<small>当前筛选条件下没有记录。</small></div>}</div></section><footer>RUNERGY · Daily Inbound Verification <span>润阳 · 每日入库核验 · Select and verify the scan worksheet</span></footer>
    </section></main>;
}
